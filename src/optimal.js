...
const key = (b) =>
	b
		.map((t) => t.map((c) => String.fromCharCode(65 + c)).join(''))
		.sort()
		.join('|');
...
export function optimal(board, report = () => {}, cap = CAP, limits = {}) {
	let bound = lowerBound(board),
		visits = 0,
		last = 0;
	const path = [];
	for (;;) {
		...
		const seen = new Map();
		function search(b, g) {
			if (visits >= (limits.maxVisits ?? Infinity) || Date.now() >= (limits.deadline ?? Infinity)) {
			...
			if (f > bound) return f;
			if (won(b, cap)) return -1;
			const k = key(b),
				old = seen.get(k);
			if (old !== undefined && old <= g) return Infinity;
			if (seen.size > 150000) seen.clear();
			seen.set(k, g);
			if (++visits % 4096 === 0 && Date.now() - last > 1000) {
				last = Date.now();
				report({ minimum: bound, exact: false });
			}
			let next = Infinity;
			...
			for (let a = 0; a < b.length; a++)
				for (let z = 0; z < b.length; z++) {
					...
					const m = move(b, a, z, cap);
					if (!m) continue;
					const ck = key(m.board);
					if (unique.has(ck)) continue;
					unique.add(ck);
		...
		const result = search(board, 0);
		if (result === -1) {
			report({ minimum: path.length, exact: true, path: path.map((m) => m.slice()) });
			return path.length;
		}
		if (!Number.isFinite(result)) {
			const error = new Error('No solution found');
			error.code = 'UNSOLVABLE';
			throw error;
		}
		bound = result;
	}
